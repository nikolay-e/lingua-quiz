...
async function crawlPage(page, path) {
  ...
  page.on('response', (response) => {
    ...
    if (status >= 400 && !url.includes('/api/tts/')) {
  ...
  try {
    ...
    try {
      const axeResults = await new AxeBuilder({ page }).withTags(['wcag2a', 'wcag2aa', 'best-practice']).analyze();
    ...
    console.log(`  ${statusCode} ${path} | errors:${errorCount} axe:${axeCount} links:${links.length}`);
  } catch (err) {
...
function printSectionErrors() {
  ...
  if (results.axeViolations.length > 0) {
    ...
    for (const [, v] of grouped) {
      console.log(`  [${v.impact}] ${v.id}: ${v.description} (${v.totalNodes} nodes on ${v.pages.length} pages)`);
    }
  }
