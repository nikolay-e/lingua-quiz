...
export function useQuizSession(
  ...
  const resetQuizSession = (clearAnswer = true) => {
    fb.resetFeedback();
    if (clearAnswer) input.setUserAnswer('');
    input.setAwaitingNextInput(false);
  };

  const applyResultExamples = (result: {
    translation: { sourceUsageExample?: string | null; targetUsageExample?: string | null };
  }) => {
  ...
  const applyLevelChange = (result: { levelChange?: { from: string; to: string } }) => {
  ...
  const handleQuizSelect = async (quiz: string): Promise<void> => {
    reset();
    resetQuizSession();
    if (quiz === '') return;
